...
import { detectPlatform, hasDocker, writeOnebotConfig, writeDockerCompose, fetchLatestReleaseAsset, downloadFile, DEFAULT_ONEBOT_PORT, } from '../napcat';
...
async function installLinuxNative() {
    ...
    if (r.status !== 0)
        throw new Error('napcat installer failed');
}
...
async function install(options) {
    ...
    if (options.native) {
        if (platform === 'linux')
            return await installLinuxNative();
        if (platform === 'win32')
            return await installWindowsNative();
...
export function napcatCommand() {
    ...
    cmd.addCommand(new Command('install')
        ...
        .action(install));
    cmd.addCommand(new Command('config')
